...
class KijijiSpider(scrapy.Spider):
    ...
    def parse_unit(self, response):
        ...
        upload_time = response.css("div.datePosted-1350605722")[0].css("span::attr(title)").extract_first()
        if upload_time is None:
            upload_time = response.css("div.datePosted-1350605722")[0].css("time::attr(title)").extract_first()
        yield {
            "title": title,
            "url": response.url,
            "price": price,
